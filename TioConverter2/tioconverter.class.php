...
class tioParser {
	...
	private $search_threshold_bracket = 40;
	private $search_threshold_tag = 70;
	private $search_threshold_name = 70;
	private $search_threshold_tag_team = 50;
	private $search_threshold_name_team = 70;
						...
						if (!$scan_complete) {
							...
							if (isset($all_matches[$mi])) {
								...
								foreach ($all_matches[$mi] as $match) {
									...
									if ($match['winner'] != "00000000-0000-0000-0000-000000000000" && isset($all_matches[$mi + 1]) && $placing > 1) {
	...
	public function search($search) {
		...
		foreach ($this->getTournaments() as $id=>$tournament) {
			...
			if ($similarity >= $this->search_threshold_bracket) {
				array_push($added_tournament_ids, $id);
		...
		foreach ($this->getFiles() as $tid=>$files) {
			if (file_exists($files['info'])) {
				...
				foreach($cached_players as $pid=>$player) {
					...
					similar_text($search, $player['tag'], $similarity_name);
					
					if ($similarity_tag >= $this->search_threshold_tag || $similarity_name >= $this->search_threshold_name) {
						
						// Add the tournament they were in if it wasn't already added
						if (!in_array($tid, $added_tournament_ids)) {
				...
				foreach($cached_teams as $pid=>$team) {
					...
					similar_text($search, $team['tag'], $similarity_name);
					
					if ($similarity_tag >= $this->search_threshold_tag_team || $similarity_name >= $this->search_threshold_name_team) {
